...
const AI_DETECTION_SCHEMA = z.object({
    score: z.number().min(0).max(1),
    label: z.enum(['likely-ai', 'likely-human', 'uncertain']),
    notes: z.string(),
});
...
function heuristicAiDetection(text: string): AiDetectionResult {
    if (!text || text.trim().length < 50) {
        return { score: 0.5, label: 'uncertain', notes: 'Essay too short for reliable detection.' };
    }
    ...
    const markerHits = aiMarkers.filter((m) => lowerText.includes(m)).length;
    const markerScore = Math.min(1, markerHits / 4);

    const lenPenalty = (avgLen > 18 && avgLen < 28) ? 0.15 : 0;
    const richnessBenefit = uniqueRatio > 0.55 ? -0.1 : 0;

    const raw = clampScore(0.3 + markerScore * 0.5 + lenPenalty + richnessBenefit);
    const label: AiDetectionResult['label'] = raw >= 0.65 ? 'likely-ai'
        : raw <= 0.35 ? 'likely-human'
        : 'uncertain';
    const notes = `Heuristic analysis: ${markerHits} AI-typical phrase${markerHits !== 1 ? 's' : ''} detected. Vocabulary richness: ${(uniqueRatio * 100).toFixed(0)}%.`;

    return { score: raw, label, notes };
}
...
export function createScoringService(
    ...
    async function detectAiContent(text: string): Promise<AiDetectionResult> {
        if (!openai || !text || text.trim().length < 50) {
        ...
        try {
            const prompt = [
                ...
                'Analyze the following essay text and determine the probability that it was generated by an AI language model (like ChatGPT, Gemini, or Claude).',
                'Consider: uniform sentence structure, lack of personal voice, overly formal tone, generic phrasing, absence of specific personal details, and repetitive patterns.',
                'Return a score from 0.0 (definitely human) to 1.0 (definitely AI), a label, and brief notes explaining your reasoning in 1-2 sentences.',
                'Labels: "likely-ai" (score >= 0.65), "likely-human" (score <= 0.35), "uncertain" (between 0.35 and 0.65)',
                '',
                'Essay:',
                text.slice(0, 3000),
            ].join('\n');
    ...
    return {
        ...
        async getExplanation(
            ...
            const essayText = intake.essay?.text ?? '';
            const aiDetection = intake.consent.aiAnalysis
                ? await detectAiContent(essayText)
                : undefined;
